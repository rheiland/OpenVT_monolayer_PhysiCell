...
class Agent:
    ...
    def divide(self, separation: float = 0.5, noise: float = 0.1):
        ...
        if daughter_split == 0:     # PhysiCell: daughter = parent x,y
            d1 = Agent(x=self.x,      y=self.y,      vel_x=0, vel_y=0, vel_x_prev=0,vel_y_prev=0,
                    area=daughter_area, growth_rate=self.growth_rate,
                    division_area=78.54 * nr1, norm_rand=nr1, ID=self.ID)
            d2 = Agent(x=self.x + xvec, y=self.y + yvec, vel_x=0, vel_y=0, vel_x_prev=0,vel_y_prev=0,
                    area=daughter_area, growth_rate=self.growth_rate,
                    division_area=78.54 * nr2, norm_rand=nr2, ID=max_ID)
        elif daughter_split == 1:   # equi-split
            xvec /= 2
            yvec /= 2
            d1 = Agent(x=self.x + xvec,      y=self.y + yvec,      vel_x=0, vel_y=0, vel_x_prev=0,vel_y_prev=0,
                    area=daughter_area, growth_rate=self.growth_rate,
                    division_area=78.54 * nr1, norm_rand=nr1, ID=self.ID)
            d2 = Agent(x=self.x - xvec, y=self.y - yvec, vel_x=0, vel_y=0, vel_x_prev=0,vel_y_prev=0,
                    area=daughter_area, growth_rate=self.growth_rate,
...
class _NpState:
    ...
    def load_from_agents(self, agents: list):
        ...
        for i, a in enumerate(agents):
            ...
            self.vx[i]            = a.vel_x
            self.vy[i]            = a.vel_y
            self.vx_prev[i]       = a.vel_x_prev
            self.vy_prev[i]       = a.vel_y_prev
            self.area[i]          = a.area
...
class Simulation:
    ...
    def __init__(
        ...
        self.agents: list[Agent] = [
            Agent(
                x=0, y=0, vel_x=0, vel_y=0, vel_x_prev=0, vel_y_prev=0, 
                area=initial_area,
    ...
    def _update_velocities_np(self, cell_cell_repulsion_strength: float = 10.0):
        ...
        x  = s.x[:n]
        print(f"_update_velocities_np: x={x}")
        y  = s.y[:n]
        r  = np.sqrt(s.area[:n] / math.pi)   # radii  (n,)

        # Pairwise displacements  (n, n)
        dx   = x[:, None] - x[None, :]       # (n, n)
        print(f"_update_velocities_np: dx={dx}")
        dy   = y[:, None] - y[None, :]
        print(f"_update_velocities_np: dy={dy}")
        ...
        print(f"_update_velocities_np: s.vx={s.vx}")
    ...
    def step(self) -> int:
        ...
        for idx in range(2):   # 10 due to 0.1 dt_mech
...
if __name__ == "__main__":
    ...
    # sim.precompute(steps=500, max_agents=MAX_AGENTS)  # it takes ~500 to have a division
    sim.precompute(steps=5, max_agents=MAX_AGENTS)  # it takes ~500 to have a division
